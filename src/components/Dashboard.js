...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/organisation/hospital-id/${hospitalId}`)
      .then(res => {
  ...
  useEffect(() => {
    if (!hospitalId) return;
    fetch(`${process.env.REACT_APP_API_URL}/api/blood-inventory/${hospitalId}`)
      .then(res => res.json())
      .then(data => {
...
const handleSaveChanges = async () => {
  ...
  try {
    ...
    await fetch(`${process.env.REACT_APP_API_URL}/api/blood-inventory/update-units/${hospitalId}`, {
      method: 'PUT',
