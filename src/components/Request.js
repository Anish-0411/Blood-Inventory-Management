...
useEffect(() => {
    const fetchHospitals = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/hospitals`); // Confirmed correct
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/requests`, newRequest);
  ...
  const handleRequestAction = async (id, action) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/requests/${id}`, { status: action });
