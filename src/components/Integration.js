...
function Integration() {
  ...
  useEffect(() => {
    const fetchSystems = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/hospitals`);
  ...
  const handleAction = async (action, id) => {
    try {
      const endpoint =
        action === 'Sync Now'
          ? `${process.env.REACT_APP_API_URL}/api/integration/sync/${id}`
          : `${process.env.REACT_APP_API_URL}/api/integration/retry/${id}`;
  ...
  const handleOpenInventoryModal = async (system) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/blood-inventory/${system.hospitalId}`);
